...
def send_department_to_other_api(sender, instance, created, **kwargs):
    if created:  # faqat yangi ma'lumot qo'shilganda ishlaydi
        url = 'http://localhost:8002/afd-platform/backend/urls/departments/'  # Boshqa loyihangizning API endpointi
        data = {
...
class Add_movies(models.Model):
    ...
    # Avtomatik URL saqlash uchun maydon
    generated_url = models.CharField(max_length=512, null=True, blank=True)

    def save(self, *args, **kwargs):
        if not self.generated_url:  # Faqat URL bo'sh bo'lsa
            slug = slugify(self.movies_name)
            self.generated_url = f"/movies/{slug}-{self.pk}"
